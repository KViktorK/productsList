...
let HeaderComponent = class HeaderComponent {
    constructor(_localStorage, _toggle, _ptoductService) {
...
let ProductDetailComponent = class ProductDetailComponent {
    constructor(dialogService, _productService, _route, _router, _toggleService, _localStorage) {
...
let ProductEditComponent = class ProductEditComponent {
    constructor(_route, _productService, _router) {
        this._route = _route;
        this._productService = _productService;
        this._router = _router;
        this.editMode = false;
    ...
    ngOnInit() {
        src_app_shared_typeList__WEBPACK_IMPORTED_MODULE_4__["typeList"].map((item) => {
    ...
    initForm() {
        ...
        let productType = "";
        let productPrices = new _angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormArray"]([]);
        if (this.editMode) {
...
let ProductListComponent = class ProductListComponent {
    constructor(_productService, _localStorage, _toggleService, _router, _route) {
        this._productService = _productService;
        this._localStorage = _localStorage;
        this._toggleService = _toggleService;
        this._router = _router;
    ...
    ngOnInit() {
        ...
        this.subscription = this._productService.productsChanged.subscribe((product) => {
            this.products = product;
        });
        this.products = this._productService.getProducts();
        this.filter = this._productService.filter;
        this._productService.search.subscribe((val) => {
            this.searchKey = val;
        });
    }
    filterChange() {
...
class Product {
    constructor(name, description, imagePath, prices, type) {
        this.name = name;
...
let ProductService = class ProductService {
    ...
    getProducts() {
        return this.products.filter(x => (x.type === 'phone' && this.filter.phone)
...
let FilterPipe = class FilterPipe {
    transform(items, searchText) {
        const arr = items.map((e) => { return e; });
        if (!arr) {
            return items;
        }
        if (!searchText) {
